...
from RLoKi_asymptotics import determine_critical_chi_asymp
import pickle 
...
def crit_chi_against_mu(Psi, epsilon, nsamp, step_size_criterion = 3.90625e-05, calculate = True):
    ...
    if(calculate == False):
        ...
        mus = np.linspace(0.8*mu_max,mu_max,nsamp)
        ...
        with open('Data/asymp_chi_crit_Psi_' + str(Psi) + '_eps_' + str(epsilon) + '_zoom' + '.pkl', 'wb') as f:
...
def crit_chi_against_a0(mu, epsilon, a0_min, a0_max, nsamp, step_size_criterion = 3.90625e-05, calculate = True):
    ...
    if(calculate == False):
        ...
        with open('Data/asymp_chi_crit_mu_' + str(mu) + '_eps_' + str(epsilon) + '.pkl', 'rb') as f:
            results_dict = pickle.load(f)
